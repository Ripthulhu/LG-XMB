...
(async()=>{
 ...
 try{
  ...
  await menu.item(page,'settings','sound');await page.keyboard.press('Enter');
  assert.equal(await page.evaluate(()=>C5App.getState().modal),'sound');
  await page.getByRole('group',{name:'Navigation sound',exact:true}).getByRole('button',{name:'On',exact:true}).click();await page.waitForFunction(()=>C5App.getState().sounds.phase==='ready');
  ...
  for(let i=0;i<10&&await page.locator(':focus').getAttribute('id')!=='reloadSounds';i++)await page.keyboard.press('ArrowDown');
  assert.equal(await page.locator(':focus').getAttribute('id'),'reloadSounds');checks.push('Reload is reachable with remote navigation');
  ...
  await select(page,'settings');await page.keyboard.press('Enter');await page.getByRole('group',{name:'Navigation sound',exact:true}).getByRole('button',{name:'Off',exact:true}).click();await clear(page);await page.keyboard.press('Escape');await page.keyboard.press('ArrowRight');await page.waitForTimeout(30);
  assert.deepEqual(await played(page),[]);checks.push('toggle Off silences navigation immediately');
  await select(page,'settings');await page.keyboard.press('Enter');missing=new Set(['snd_cancel.wav']);await page.getByRole('group',{name:'Navigation sound',exact:true}).getByRole('button',{name:'On',exact:true}).click();
