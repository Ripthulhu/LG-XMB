...
const root={};vm.runInNewContext(fs.readFileSync(path.join(__dirname,'../app/wave-colors.js'),'utf8'),{window:root});const colors=root.LGXMBWaveColors;
...
test('the ps3 colour source resolves to a monthly request with a near-white wave and no gradient endpoints',()=>{
  ...
  assert.deepEqual(plain(auto.monthly),{auto:true,month:1,period:'day'});assert.equal(auto.start,undefined);
