...
(() => {
  ...
  window.LGXMBWaveColors = {normalize:v=>v||{},resolve:()=>null,menuGradient:()=>''};
  ...
  h.builtins = C5Catalog.flatMap(c => c.items.filter(i => !i.action).map(i => ({id:i.id,title:i.title})))
    .filter((item,index,all) => all.findIndex(i => i.id===item.id)===index);
  let appReply = null, appValue = {preview:false,apps:h.builtins}, appError = null;
  h.apps = value => {appValue=value;appError=null;if(appReply){const r=appReply;appReply=null;r.resolve(value);}else if(window.C5App)document.dispatchEvent(new Event('webOSRelaunch'));};
  ...
  window.C5TV = {
    ...
    listInputs:()=>new Promise((resolve,reject)=>{h.labels=resolve;h.labelsFail=reject;}),
