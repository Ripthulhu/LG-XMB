...
(async()=>{
 ...
 const prefs=await page.evaluate(()=>JSON.parse(localStorage.getItem('lg-xmb-preferences-v1')));assert.deepEqual(Object.keys(prefs).sort(),['backBehavior','background','backgroundBrightness','colour','motion','musicEnabled','musicVolume','previewMode','screensaverBrightness','screensaverDelay','sound','theme','waveDetail','waveFrameRate','waveParticleCount','wavePostprocess','waveSampling','waveSmoothing','waveSoftness','waveSpeed']);checks.push('Only appearance, wave, sound, preview and Back preferences stored; no usage history');
