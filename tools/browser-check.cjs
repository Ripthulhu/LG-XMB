...
const menu=require('../tests/support/menu-navigation.cjs');
...
(async()=>{
 ...
 let state=await page.evaluate(()=>C5App.getState());assert.equal(state.category,'tv');assert.equal(state.waveMode,'webgl');assert.equal(state.waveError,null);checks.push('Rich WebGL shader initializes after menu startup');
 ...
 await menu.item(page,'video','com.webos.app.mediadiscovery');
 const lastVideo=await page.evaluate(()=>C5Catalog.find(c=>c.id==='video').items.slice(-1)[0].id);
 await menu.item(page,'video',lastVideo);await page.keyboard.press('ArrowDown');
 assert.equal((await page.evaluate(()=>C5App.getState())).item,lastVideo);
 await menu.item(page,'video','com.webos.app.mediadiscovery');
 await menu.item(page,'tv','com.webos.app.hdmi1');
 assert.equal((await page.evaluate(()=>C5App.getState())).category,'tv');
 ...
 await menu.category(page,'video');assert.equal((await page.evaluate(()=>C5App.getState())).item,'com.webos.app.mediadiscovery');checks.push('Category/item bounds and per-category selection memory');
 ...
 await page.keyboard.down('Enter');assert.equal(await page.evaluate(()=>window.repeatLaunches),0);await page.keyboard.up('Enter');
 await page.waitForFunction(()=>!C5App.getState().busy);assert.match(await page.locator('#toast').innerText(),/Preview/);checks.push('Short OK launches on release and browser launch is explicitly preview-only');
 await page.keyboard.down('Enter');await page.waitForFunction(()=>C5App.getState().itemOptions.open);
 await page.keyboard.down('Enter');await page.keyboard.up('Enter');assert.equal(await page.evaluate(()=>window.repeatLaunches),1);
 await page.keyboard.press('Escape');assert.equal((await page.evaluate(()=>C5App.getState())).modal,null);
 await page.evaluate(()=>{window.C5TV=window.repeatTestBridge;delete window.repeatTestBridge;delete window.repeatLaunches;});checks.push('Holding OK opens options without launching, including repeats and release');
 await menu.item(page,'settings','appearance');await page.keyboard.press('Enter');assert.equal((await page.evaluate(()=>C5App.getState())).modal,'appearance');
 await page.getByRole('button',{name:'Theme',exact:true}).click();assert.equal((await page.evaluate(()=>C5App.getState())).modal,'theme');
 await page.getByRole('button',{name:'Classic',exact:true}).click();assert.equal((await page.evaluate(()=>C5App.getState())).preferences.theme,'classic');
 await page.keyboard.press('Escape');assert.equal((await page.evaluate(()=>C5App.getState())).modal,'appearance');
 ...
 await page.reload();await page.waitForFunction(()=>window.C5App);assert.equal((await page.evaluate(()=>C5App.getState())).preferences.theme,'classic');checks.push('Theme persists locally; dialog focus stays contained; Back closes it');
 await page.waitForTimeout(500);await page.screenshot({path:path.join(dir,'classic-1080.png')});
 await menu.item(page,'settings','appearance');await page.keyboard.press('Enter');
 await page.locator('#openAppearanceAdvanced').click();
 await page.getByRole('group',{name:'Animation',exact:true}).getByRole('button',{name:'Off',exact:true}).click();assert.equal((await page.evaluate(()=>C5App.getState())).preferences.motion,'reduced');await page.keyboard.press('Escape');await page.keyboard.press('Escape');checks.push('Background animation can be disabled');
 ...
 assert.equal(afterMainBack.toast,beforeMainBack.toast);assert.equal(afterMainBack.toastVisible,beforeMainBack.toastVisible);checks.push('Desktop-preview Back sends no TV launch and shows no toast, including TV keycode 461');
 await page.evaluate(()=>{window.C5TV=window.mainBackBridge;delete window.mainBackBridge;delete window.mainBackCalls;});
 assert.equal(await page.evaluate(()=>C5Catalog.find(category=>category.id==='settings').items.some(item=>item.id==='about')),false);checks.push('Settings has no About item');
 const prefs=await page.evaluate(()=>JSON.parse(localStorage.getItem('lg-xmb-preferences-v1')));assert.deepEqual(Object.keys(prefs).sort(),['backBehavior','background','backgroundBrightness','clockStyle','colour','motion','musicEnabled','musicVolume','previewMode','screensaverBrightness','screensaverDelay','screensaverParticleBrightness','screensaverWaveBrightness','sound','theme','waveDetail','waveFrameRate','waveParticleCount','wavePostprocess','waveSampling','waveSmoothing','waveSoftness','waveSpeed']);checks.push('Only appearance, wave, sound, preview and Back preferences stored; no usage history');
