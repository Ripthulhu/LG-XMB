...
(async()=>{
 ...
 assert.equal((await page.evaluate(()=>C5App.getState())).item,'about');await page.keyboard.press('Enter');assert.equal((await page.evaluate(()=>C5App.getState())).modal,'about');assert.match(await page.locator('#modalIntro').innerText(),/\b0\.1\.20\b/);await page.keyboard.press('Escape');checks.push('About identifies version 0.1.20');
