...
(async()=>{
 ...
 const prefs=await page.evaluate(()=>JSON.parse(localStorage.getItem('lg-xmb-preferences-v1')));assert.deepEqual(Object.keys(prefs).sort(),['backBehavior','motion','musicEnabled','musicVolume','previewMode','sound','theme','waveBrightness','waveColors','waveDetail','waveFrameRate','waveParticleCount','waveParticles','wavePostprocess','waveSampling','waveSmoothing','waveSoftness','waveSpeed']);checks.push('Only appearance, wave, sound, preview and Back preferences stored; no usage history');
