...
(function (root) {
  ...
  function ItemOptions(options) {
    ...
    this.element.className = 'item-options';
    this.element.innerHTML =
      '<div class="modal-backdrop item-options-shade"></div><section class="modal item-options-panel" role="dialog" aria-modal="true" aria-labelledby="itemOptionsTitle" tabindex="-1">' +
    ...
    this.element.querySelector('.item-options-shade').addEventListener('click', function () {
      self.close('back');
    });
  }
  ItemOptions.prototype.sound = function (name) {
    this.options.sound(name);
  };
  ItemOptions.prototype.button = function (label, action, handler, disabled) {
    var b = root.document.createElement('button');
    b.type = 'button';
    b.className = 'option item-options-button';
    b.textContent = label;
    ...
    b.addEventListener('click', function () {
      ...
      if (b.getAttribute('aria-disabled') === 'true') {
        self.explainRemoval = action === 'delete';
        self.status.textContent =
          DISABLED_ACTION_MESSAGES[action] || self.reason || 'Not available for this item.';
        return;
      }
      handler();
    });
  ...
  ItemOptions.prototype.prepare = function (item, category) {
    ...
    this.categoryDraft = null;
    this.render();
    this.panel.querySelectorAll('button').forEach(function (b) {
      b.tabIndex = -1;
    });
    ...
    this.element.classList.add('prepared');
  ...
  ItemOptions.prototype.open = function (item, category) {
    ...
    this.element.classList.remove('prepared');
  ...
  ItemOptions.prototype.loadInfo = function () {
    var self = this,
      generation = this.generation,
      request = this.manager.getAppInfo(this.item.id);
    ...
    request.then(
      ...
      function (e) {
        if (!self.opened || generation !== self.generation || self.request !== request) return;
        self.request = null;
        self.error = e.message || 'App information is unavailable.';
        self.refreshInfo();
      }
    );
  };
  ItemOptions.prototype.refreshInfo = function () {
    ...
    this.reason = this.info
      ? this.info.removalReason
      : this.error || 'Checking whether this app can be deleted…';
    var b = this.actions.querySelector('[data-action=delete]');
    if (b) {
      b.setAttribute('aria-disabled', this.info && this.info.removable ? 'false' : 'true');
    }
    if (this.view === 'info') this.renderInfo();
    else if (this.view === 'main' && this.explainRemoval)
      this.status.textContent = this.info && this.info.removable ? '' : this.reason;
  };
  ItemOptions.prototype.render = function () {
    ...
    if (
      this.view !== 'main' ||
      !this.mainButtons ||
      this.mainButtons[0].parentNode !== this.actions
    )
    ...
    this.caption.textContent = this.category.title;
    switch (this.view) {
      case 'main':
        this.renderMain();
        break;
  ...
  // Keep the main action nodes alive between openings: metadata must not move focus.
  ItemOptions.prototype.renderMain = function () {
    var self = this,
      item = this.item;
    this.reason = item.action
      ? 'Inputs and launcher settings cannot be uninstalled.'
      : this.info
        ? this.info.removalReason
        : this.error || 'Checking whether this app can be deleted…';
    if (!this.mainButtons) {
      this.button('Sort By', 'sort', function () {
        self.view = 'sort';
        self.render();
        self.focus();
        self.sound('option');
      ...
      this.button('Start', 'start', function () {
        var target = self.item;
        self.close('start');
        self.options.onStart(target);
      });
      this.button('Hide app', 'hide', function () {
        try {
          self.options.onHide(self.item);
          self.sound('decide');
          self.close('hidden');
        } catch (error) {
          self.status.textContent = 'Could not hide the app. ' + error.message;
          self.sound('error');
        }
      });
      this.button('Show hidden apps', 'hidden', function () {
        self.view = 'hidden';
        self.hiddenChanged = false;
        self.render();
        self.focus();
        self.sound('option');
      });
      this.button('Delete', 'delete', function () {
        self.view = 'confirm';
        self.render();
        self.focus();
        self.sound('option');
      });
      this.button('Information', 'info', function () {
        self.view = 'info';
        self.render();
        self.focus();
        self.sound('option');
      });
      this.button('Refresh apps', 'refresh', function () {
        self.close('refresh');
      ...
      this.mainButtons.forEach(function (b) {
        self.actions.appendChild(b);
      });
    }
    this.actions
      .querySelector('[data-action=sort]')
      .setAttribute('aria-disabled', String(this.category.items.length < 2));
    this.actions
    ...
    this.actions
      ...
      .setAttribute('aria-disabled', String(!this.options.canHide || !this.options.canHide(item)));
    this.actions
      .querySelector('[data-action=hidden]')
      .setAttribute('aria-disabled', String(!this.options.getHidden));
    this.actions
      .querySelector('[data-action=delete]')
      .setAttribute('aria-disabled', String(!this.info || !this.info.removable));
    this.actions
      .querySelector('[data-action=refresh]')
      .setAttribute('aria-disabled', String(!this.options.onRefresh));
    this.mainButtons.forEach(function (b) {
      b.tabIndex = self.opened ? 0 : -1;
    });
  ...
  ItemOptions.prototype.renderSort = function () {
    var self = this,
      item = this.item;
    ...
    [
      ...
      var b = self.button(choice[1], 'sort-' + choice[0], function () {
        try {
          self.options.onSort(self.category, choice[0], item.id);
          self.sound('decide');
          self.view = 'main';
          self.render();
          self.focus();
        } catch (error) {
          self.status.textContent = 'Could not save the sort order. ' + error.message;
          self.sound('error');
        }
      });
      b.setAttribute(
        'aria-pressed',
        self.options.getSort(self.category.id) === choice[0] ? 'true' : 'false'
      );
    });
  };

  ItemOptions.prototype.renderHiddenApps = function () {
    var self = this;
    this.title.textContent = 'Hidden apps';
    this.caption.textContent = 'Restore an app';
    var hidden = this.options.getHidden();
    hidden.forEach(function (entry, index) {
      var b = self.button(entry.title, 'restore', function () {
        try {
          self.options.onRestore(entry.id);
          self.hiddenChanged = true;
          self.render();
          var buttons = self.actions.querySelectorAll('button');
          root.LGXMBMenuFocus(buttons[Math.min(index, buttons.length - 1)] || self.panel);
          self.sound('decide');
        } catch (error) {
          self.status.textContent = 'Could not restore the app. ' + error.message;
          self.sound('error');
        }
      });
      b.dataset.appId = entry.id;
      b.setAttribute('aria-label', 'Restore ' + entry.title);
    });
    if (!hidden.length) {
  ...
  ItemOptions.prototype.renderCategories = function () {
    ...
    this.options.getCategories().forEach(function (choice) {
      var b = self.button(choice.title, 'category-' + choice.id, function () {
        ...
        if (self.categoryDefault)
          self.categoryDraft = self.categoryDraft.filter(function (id) {
            return self.options.getCategories().some(function (c) {
              return c.id === id;
            });
          });
        var index = self.categoryDraft.indexOf(choice.id);
        if (index < 0) self.categoryDraft.push(choice.id);
        else self.categoryDraft.splice(index, 1);
        self.categoryDefault = false;
        self.updateCategoryChecks();
        self.sound('cursor');
      });
    ...
    this.button('Apply categories', 'category-apply', function () {
      try {
        self.options.onCategory(
          item,
          self.categoryDefault ? 'default' : self.categoryDraft.slice()
        );
        self.sound('decide');
        self.close('category');
      } catch (error) {
        self.status.textContent = error.message || 'Could not save the categories.';
        self.sound('error');
      }
    });
    this.button('Default locations', 'category-default', function () {
      self.categoryDraft = self.options.getDefaultCategories(item.id).slice();
      self.categoryDefault = true;
      self.updateCategoryChecks();
      self.sound('cursor');
    });
    this.button('Cancel', 'category-cancel', function () {
      self.back();
    });
    this.updateCategoryChecks();
  };

  ItemOptions.prototype.renderDeleteConfirmation = function () {
    var self = this,
      item = this.item;
    this.caption.textContent = 'Delete this app from the TV?';
    var p = root.document.createElement('p');
    p.className = 'item-options-warning';
    p.textContent =
      'This uninstalls “' +
      item.title +
      '”, not just its shortcut. Local app data may also be removed.';
    this.content.appendChild(p);
    this.button('Cancel', 'cancel-delete', function () {
      self.back();
    });
    this.button('Delete app', 'confirm-delete', function () {
      self.deleteConfirmed();
    });
    this.status.textContent = 'You can reinstall it later from its store.';
  };

  ItemOptions.prototype.renderInformation = function () {
    var self = this;
    this.caption.textContent = 'Application information';
    this.renderInfo();
    this.button('Back', 'back', function () {
      self.back();
    });
  };
  ...
  ItemOptions.prototype.updateCategoryChecks = function () {
    ...
    this.actions
      .querySelector('[data-action=category-apply]')
      .setAttribute('aria-disabled', String(!selected.length));
    this.actions
      .querySelector('[data-action=category-default]')
      .setAttribute('aria-pressed', String(this.categoryDefault));
    this.status.textContent = !selected.length
      ? 'Choose at least one category, or restore the default locations.'
      : this.categoryDefault
        ? 'Default locations selected. Choose Apply categories to restore them.'
  ...
  ItemOptions.prototype.renderInfo = function () {
    ...
    var i = this.info || {},
      item = this.item,
      dl = root.document.createElement('dl');
    dl.className = 'item-options-info';
    var locations =
      this.options.getCategoryLocations && this.options.canAssign && this.options.canAssign(item)
        ? this.options.getCategoryLocations(item.id)
        : [this.category.id];
    var choices = this.options.getCategories ? this.options.getCategories() : [this.category];
    var names = choices
      .filter(function (c) {
        return locations.indexOf(c.id) >= 0;
      })
      .map(function (c) {
        return c.title;
      });
    [
      ['Name', i.title || item.title],
      ['Categories', names.join(', ') || this.category.title],
      ['App ID', item.action && item.action !== 'input' ? 'Launcher setting' : item.id],
      ['Version', i.version || 'Not reported'],
      ['Developer', i.vendor || 'Not reported'],
      ['Type', i.type || item.type],
      ['Installation', i.installation || (item.action ? 'Built-in shortcut' : 'Not reported')],
      ['Description', i.description || item.description || 'Not reported']
    ].forEach(function (pair) {
      var dt = root.document.createElement('dt'),
        dd = root.document.createElement('dd');
  ...
  ItemOptions.prototype.deleteConfirmed = function () {
    ...
    var self = this,
      ...
      generation = this.generation;
    this.sound('decide');
    this.status.textContent = 'Deleting ' + title + '…';
    this.actions.querySelectorAll('button').forEach(function (b) {
      b.setAttribute('aria-disabled', 'true');
    });
    var op = (this.removal = this.manager.removeApp(id, true));
    this.panel.setAttribute('aria-busy', 'true');
    op.then(
      function (result) {
        self.removal = null;
        self.panel.removeAttribute('aria-busy');
      ...
      function (e) {
        ...
        self.render();
        self.focus();
        self.status.textContent = e.message || 'Deletion failed.';
        self.sound('error');
      }
    );
  };
  ItemOptions.prototype.back = function () {
    ...
    if (this.view === 'hidden' && this.hiddenChanged && this.item.action === 'empty') {
      this.close('restored');
      return;
    }
    if (this.view !== 'main' && !this.removal) {
      this.categoryDraft = null;
      this.view = 'main';
      this.render();
      this.focus();
  ...
  ItemOptions.prototype.close = function (reason) {
    if (!this.opened) return;
    root.clearTimeout(this.infoTimer);
    this.infoTimer = null;
    this.opened = false;
    this.generation++;
    this.categoryDraft = null;
  ...
  ItemOptions.prototype.key = function (e) {
    ...
    if (enter) {
      e.preventDefault();
      if (!e.repeat && this.panel.contains(root.document.activeElement))
        root.document.activeElement.click();
      return;
    }
    if (
      e.key === ' ' &&
      ...
      root.document.activeElement.getAttribute('role') === 'checkbox'
    ) {
      e.preventDefault();
      if (!e.repeat) root.document.activeElement.click();
      return;
    }
    if (e.key === 'ArrowRight') {
      e.preventDefault();
    ...
    if (direction) {
      ...
      // Arrows stop at the edges; Tab cycles within the dialog.
      var next =
        e.key === 'Tab'
          ? (Math.max(0, index) + direction + buttons.length) % buttons.length
          : Math.max(0, Math.min(buttons.length - 1, index + direction));
      if (root.LGXMBMenuFocus(buttons[next])) this.sound('cursor');
    }
  };
  ItemOptions.prototype.getState = function () {
    return {
      open: this.opened,
      opening: false,
      view: this.view,
      item: this.item && this.item.id,
      deleting: !!this.removal,
      removable: !!(this.info && this.info.removable)
    };
  };
  ItemOptions.prototype.destroy = function () {
    this.close('lifecycle');
    root.clearTimeout(this.infoTimer);
    this.element.remove();
  };
