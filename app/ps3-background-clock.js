...
(function (root) {
  ...
  function fromLocalDate(date) {
    ...
    return calendar(date.getMonth()+1,date.getDate(),date.getHours(),date.getMinutes(),date.getSeconds());
  }
  ...
  function uniforms(coordinates, settings, alpha = 1) {
    ...
    if (!coordinates || !Number.isFinite(coordinates.day) || !Number.isFinite(coordinates.month) ||
        !Number.isFinite(alpha) || !Object.values(s).every(Number.isFinite) ||
        s.dawnBegin >= s.dawnEnd || s.duskBegin >= s.duskEnd) {
      throw new RangeError('Invalid background coordinates or transition settings');
    }
    const t=F(Math.max(0,Math.min(1,coordinates.day))),m=F(Math.max(0,Math.min(12,coordinates.month)));
    const i=Math.trunc(m),a=i%12,b=(a+1)%12,t2=F(t*t),t3=F(t2*t),spread=F(s.daySpread);
    const h1=F(F(-2*t3)+F(3*t2)),h2=F(F(t3-F(2*t2))+t),h3=F(t3-t2);
    const q=F(F(h1+F(h2*spread))+F(h3*spread));
    let nt=F(t-0.5);if(nt<0)nt=F(nt+1);
    const rise=smooth(F(s.dawnBegin/24),F(s.dawnEnd/24),t);
    const fall=F(1-smooth(F(s.duskBegin/24),F(s.duskEnd/24),t));
    return {layers:[a,b,12+a,12+b],values:{_DayTime:F(q*2400),_NightTime:F(nt*2400),
      _MonthTime:F(F(m-i)*30),_NightDayBlend:F(1+F(F(rise*fall)-1)*F(s.nightBlend)),
      _NightBrightness:F(s.nightBrightness),_Alpha:F(alpha)}};
  }
