...
var detailItemId=null,launchGeneration=0,pageActive=!document.hidden,musicAway=false,lastReturnAt=-Infinity;
...
var modalType='';
function openModal(type){if(!modalOpen||modalType!==type)clearToast();if(modalType==='background')C5BackgroundSettings.close();if(modalType==='remote')C5RemoteSettings.close();stopInputPreview();modalType=type;$('modal').classList.toggle('background-settings',type==='background');$('modal').classList.toggle('waves-settings',type==='motion'||type==='music');$('modal').classList.toggle('appearance-settings',type==='appearance');modalOpen=true;syncInputPreview();document.querySelector('.screen').setAttribute('aria-hidden','true');$('modalBackdrop').hidden=false;$('modalContent').textContent='';$('modalContent').classList.remove('theme-options');$('modalTitle').textContent={appearance:'Appearance',motion:'Waves',sound:'Navigation sound',music:'Background music',previews:'Input previews',background:'Background activity',remote:'Remote buttons',about:'About this menu'}[type];$('modalIntro').textContent={appearance:'',motion:'',sound:'',music:'Loop your own MP3 while Home is open. Music pauses during live HDMI previews and when you leave Home.',previews:'Live previews can change HDR mode.',background:'Choose what Home keeps closed. Apps can still be opened and close again when you return.',remote:'Home applies across the TV. Back applies in this menu.',about:'Version 0.1.22'}[type];
...
function openMusicSettings(){
  ...
  path.textContent='/var/lib/lg-xmb/music/background.mp3';$('modalContent').appendChild(path);
...
function suspendPage(){var wasActive=pageActive;pageActive=false;music.setContext(false,false);cancelInputLabels();invalidateLaunch();clearToast();clearTimeout(announceTimer);if(wasActive){stopInputPreview();thumbnail.setPaused(true);wave.setPaused(true);}}
...
window.addEventListener('pagehide',suspendPage);window.addEventListener('pageshow',function(){restorePage(false);});window.addEventListener('beforeunload',function(){clearInterval(clockTimer);suspendPage();music.destroy();thumbnail.destroy();inputPreview.destroy();wave.destroy();});
