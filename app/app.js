...
async function activate(){if(busy||!pageActive||document.hidden)return;categoryTransition.cancel();var item=currentItem();if(['appearance','motion','sound','music','previews','remote','about'].indexOf(item.action)!==-1){openModal(item.action);return;}var generation=++launchGeneration;clearToast();busy=true;musicAway=true;syncInputPreview();var launched=false;$('items').setAttribute('aria-busy','true');try{var result=item.action==='stock'?await C5TV.exitToStockHome():item.action==='input'?await C5TV.openInput(item.id):await C5TV.launch(item.id);if(generation!==launchGeneration||!pageActive||document.hidden)return;launched=!result.preview;if(result.preview)toast('Preview · '+item.title+' opens on your TV.');}catch(error){if(generation===launchGeneration&&pageActive&&!document.hidden)toast('Could not open '+item.title+'. '+(error.message||'Please try LG Home.'));}finally{if(generation===launchGeneration){busy=false;$('items').removeAttribute('aria-busy');if(!launched&&pageActive&&!document.hidden){musicAway=false;renderDetail();$('items').focus();}}}}
