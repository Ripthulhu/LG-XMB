...
(function (root) {
  ...
  var f = Math.fround,
    DEAD = -666,
    AGE_LIMIT = f(0.9999899864196777);
  var scratch = new ArrayBuffer(4),
    floats = new Float32Array(scratch),
    words = new Uint32Array(scratch);
  var RECIP = [
    0x7ffbe0, 0x7f87a6, 0x70ef72, 0x708b40, 0x638b12, 0x633aea, 0x5792c4, 0x574aa0, 0x4cca7e,
    0x4c9262, 0x430a44, 0x42d62a, 0x3a2e12, 0x39fdfa, 0x3215e4, 0x31f1d2, 0x2aa9be, 0x2a85ac,
    0x23d59a, 0x23bd8e, 0x1d8576, 0x1d8576, 0x17ad5a, 0x17ad5a, 0x124543, 0x124543, 0x0d392d,
    0x0d392d, 0x08851a, 0x08851a, 0x041d07, 0x041d07
  ];
  function bits(x) {
    floats[0] = x;
    return words[0];
  ...
  function reciprocal(x) {
    var a = bits(x),
      e = (a >>> 23) & 255;
    if (e < 1 || e > 252) throw new RangeError('Reciprocal requires a finite normal input');
  ...
  function array(input, length, name) {
    ...
    if (a.length !== length || !a.every(Number.isFinite)) throw new TypeError('Invalid ' + name);
    return a;
  }
  ...
  function Wave(seed, settings) {
    ...
    this.interpolate();
    if (seed.lattice) this.lattice.set(array(seed.lattice, 2156, 'wave lattice'));
    else this.generateLattice();
    this.deform();
  }
  ...
  Wave.prototype.tick = function () {
    ...
    var rest = f(s.LENGTH),
      tension = f(s.TENSION),
      rest2 = f(2 * rest),
      tension2 = f(10 * tension);
    this.previous.set(p);
    this.clock = f(this.clock + dt);
    if (this.transitionTicks > 0) this.transitionTicks--;
    if (this.clock > 10) {
      this.clock = 0;
      this.clockWraps++;
    ...
    for (var y = 0; y < 19; y++)
      for (var x = 0; x < 19; x++) {
        var a = (y * 19 + x) * 4;
        if (x > 0) this.spring(a, a - 4, rest, tension);
        if (x > 1) this.spring(a, a - 8, rest2, tension2);
        if (y > 0) this.spring(a, a - 76, rest, tension);
        if (y > 1) this.spring(a, a - 152, rest2, tension2);
        for (var k = 0; k < 3; k++) {
    ...
    this.smoothedClock = f(f(this.clock - this.smoothedClock) * f(0.1) + this.smoothedClock);
    for (y = 0; y < 19; y++) {
      a = y * 76;
      v[a] = f(v[a] + f(step * f(0.02)));
      var phase = f(f(y / 19) + this.smoothedClock);
      a += 72;
      p[a] = 0;
      p[a + 1] = f(f(f(f(Math.sin(f(phase * 11))) + 1) * f(0.5)) * f(s['END Y']));
      p[a + 2] = f(f(f(f(Math.cos(f(phase * 15))) + 1) * f(0.5)) * f(s['END Z']));
      p[a + 3] = 1;
  ...
  Wave.prototype.interpolate = function () {
    var a = this.fraction,
      ...
      p = this.p,
      prev = this.previous,
      start = f(this.settings['FFD PARAM 1']);
    for (var i = 0; i < 1444; i += 4) {
      for (var k = 0; k < 4; k++) o[i + k] = f(p[i + k] * a + f(prev[i + k] * f(1 - a)));
      var d = f(o[i] - start),
        phase = d < 0 ? 0 : f(d * f(Math.PI / 2));
      var u = d < 0 ? 0 : f(d / 5),
        t = f(1 - Math.min(1, Math.max(0, u)));
  ...
  Wave.prototype.generateLattice = function () {
    ...
    var time = f(f(this.clock - f(f(this.settings.TIMESTEP) * f(0.0001))) * 10);
    if (time < 0) time = 0;
    var shader = ffd[this.program],
      core = this.core,
      grid = this.lattice;
    for (var z = 0; z < 4; z++)
      for (var y = 0; y < 4; y++)
    ...
    for (z = 0; z < 7; z++)
      for (y = 0; y < 7; y++)
        for (x = 0; x < 11; x++) {
          i = ((z * 7 + y) * 11 + x) * 4;
          var j =
            ((Math.min(3, Math.max(0, z - 1)) * 4 + Math.min(3, Math.max(0, y - 1))) * 8 +
              Math.min(7, Math.max(0, x - 1))) *
            4;
          for (k = 0; k < 4; k++) grid[i + k] = core[j + k];
        }
  ...
  Wave.prototype.deform = function () {
    ...
    var sixth = 0.1666666567325592,
      third = 0.6666666269302368,
      half = 0.4999999701976776;
    for (var i = 0; i < 1444; i += 4) {
      var nx = (o[i] - this.origin[0]) * this.inverse[0],
        ny = (o[i + 1] - this.origin[1]) * this.inverse[1];
      var nz = (o[i + 2] - this.origin[2]) * this.inverse[2],
        nw = (o[i + 3] - this.origin[3]) * this.inverse[3];
      var xx = Math.max(0, Math.min(0.9990000128746033, nx)) * 8;
      var yy = Math.max(0, Math.min(0.9990000128746033, ny)) * 4,
        zz = Math.max(0, Math.min(0.9990000128746033, nz)) * 4;
      var ix = Math.floor(xx),
        iy = Math.floor(yy),
        iz = Math.floor(zz);
      for (var ax = 0; ax < 3; ax++) {
        var t = ax === 0 ? xx - ix : ax === 1 ? yy - iy : zz - iz,
          t2 = t * t,
          t3 = t2 * t,
          q = ax * 4;
      ...
      for (var z = 0; z < 4; z++)
        for (var y = 0; y < 4; y++)
          for (var x = 0; x < 4; x++) {
            var weight = wt[x] * wt[4 + y] * wt[8 + z],
              j = (((iz + z) * 7 + iy + y) * 11 + ix + x) * 4;
            dx += l[j] * weight;
            dy += l[j + 1] * weight;
            dz += l[j + 2] * weight;
            dw += l[j + 3] * weight;
  ...
  Wave.prototype.advance = function (seconds) {
    ...
    this.interpolate();
    if (this.ticks !== before) this.generateLattice();
    this.deform();
  ...
  function invert4(m) {
    var a = new Float64Array(32),
      ...
      k;
    for (i = 0; i < 4; i++) {
      for (j = 0; j < 4; j++) a[i * 8 + j] = m[i * 4 + j];
      a[i * 8 + 4 + i] = 1;
    }
    for (i = 0; i < 4; i++) {
      var pivot = i;
      for (j = i + 1; j < 4; j++)
        if (Math.abs(a[j * 8 + i]) > Math.abs(a[pivot * 8 + i])) pivot = j;
      ...
      for (j = 0; j < 4; j++)
        if (j !== i) {
          var factor = a[j * 8 + i];
          for (k = 0; k < 8; k++) a[j * 8 + k] -= factor * a[i * 8 + k];
        }
    }
  ...
  function Emitter(wave, particles, basis, viewProjection, birth) {
    if (!birth || typeof birth.createBirth !== 'function')
      throw new TypeError('Particle birth module required');
    this.wave = wave;
    ...
    for (var r = 0; r < 4; r++)
      for (var q = 0; q < 4; q++) transposed[r * 4 + q] = viewProjection[q * 4 + r];
    ...
    for (var i = 0; i < particles.capacity; i++)
      if (particles.state[i * 12 + 3] === DEAD) this.free.push(i);
    var clip = new Float32Array(4),
      m = this.toWorld,
      self = this;
    this.sample = function (x, y) {
      self.wave.meshPoint(x, y, self.basis, clip);
      return [
        clip[0] * m[0] + clip[1] * m[1] + clip[2] * m[2] + clip[3] * m[3],
        clip[0] * m[4] + clip[1] * m[5] + clip[2] * m[6] + clip[3] * m[7],
        clip[0] * m[8] + clip[1] * m[9] + clip[2] * m[10] + clip[3] * m[11]
      ];
    };
  }
  Emitter.prototype.retire = function (slots, count) {
    for (var n = 0; n < count; n++) this.free.push(slots[n]);
  };
  ...
  Emitter.prototype.emit = function (updates) {
    ...
    for (n = 0; n < updates; n++) {
      Array.prototype.push.apply(
        fresh,
        B.batchCandidates(this.sample, 128, 128, null, this.signed)
      );
      Array.prototype.push.apply(
        fresh,
        B.trailCandidates(this.sample, 128, 128, this.trail, this.tick++, this.signed)
      );
    }
    var settings = updates === 1 ? null : { dt: B.defaults.dt * updates };
    for (i = 0; i < this.previous.length; i++) {
      var c = this.previous[i],
        made = B.createBirth(c.position, this.sample(c.x, c.y), settings, this.signed, [0, 0, 1]);
      if (!made) continue;
      // A full pool refuses the birth, it never overwrites a live particle.
      if (
        this.particles.capacity - this.free.length < this.limit &&
        B.applyBirth(this.particles.state, this.free, made) >= 0
      )
        born++;
      else this.refused++;
    }
  ...
  var APPROACH = 0.22058835625648499,
    ICON_ASPECT = f(1.7777777910232544),
    UI_BROWNIAN = f(0.6000000238418579);
  ...
  var VERTICAL_ICON_WIND_SCALE = 0.25;
  ...
  Interaction.prototype.direction = function (code) {
    if (code >= 0 && code <= 3) this.pending = code;
  };
  ...
  Interaction.prototype.step = function () {
    var p = this.particles.params,
      B = this.birth,
      out = this.response.step(this.pending, 1, 0);
    this.pending = 4;
    ...
    var r = out.rotation,
      ...
      m = p.rotation;
    if (angle < 1e-5) {
      for (var n = 0; n < 16; n++) m[n] = n % 5 === 0 ? 1 : 0;
    } else {
      var x = r[0] / angle,
        y = r[1] / angle,
        z = r[2] / angle,
    ...
    for (var id in this.objects) {
      ...
      var nx = o.x + (o.tx - o.x) * APPROACH,
        ny = o.y + (o.ty - o.y) * APPROACH;
      if (Math.abs(o.tx - nx) < 1e-4 && Math.abs(o.ty - ny) < 1e-4) {
        nx = o.tx;
        ny = o.ty;
      } else still = false;
      // A zero-strength move still overwrites the cell it visits.
      if (B.iconFieldWrite(p.field, [o.x, o.y], [nx, ny], ICON_ASPECT)) {
        this.writes++;
        this.fieldLive = true;
      }
      o.x = nx;
  ...
  function Parameters(raw) {
    ...
    function read(at, count) {
      var a = new Float32Array(count);
      for (var i = 0; i < count; i++) a[i] = d.getFloat32(at + i * 4);
      return a;
    }
    this.force = read(0, 3);
    this.drag = read(16, 3);
    ...
    if (
      !this.dt.every(function (x) {
        return x > 0;
      }) ||
      !this.minimum.every(function (x, i) {
        return x <= this.maximum[i];
      }, this)
    )
  ...
  function Particles(records, parameters) {
    ...
    for (var i = 0; i < this.capacity; i++)
      if (this.seeds[i * 12 + 3] !== DEAD) this.seedIndices[this.seedCount++] = i;
    this.pack();
  }
  Particles.prototype.sampleField = function (px, py, pz) {
    ...
    var u = f(t[0] * 32 - 0.5),
      ...
      fx = f(u - x),
      fy = f(w - y);
    var x0 = Math.max(0, Math.min(31, x)),
      x1 = Math.max(0, Math.min(31, x + 1));
    var y0 = Math.max(0, Math.min(15, y)),
      y1 = Math.max(0, Math.min(15, y + 1)),
      scale = f(1 / 127);
    for (var k = 0; k < 3; k++) {
      var a = f(field[(y0 * 32 + x0) * 3 + k] * scale),
        b = f(field[(y0 * 32 + x1) * 3 + k] * scale);
      var c = f(field[(y1 * 32 + x0) * 3 + k] * scale),
        d = f(field[(y1 * 32 + x1) * 3 + k] * scale);
      var lo = f(f(b - a) * fx + a),
        hi = f(f(d - c) * fx + c);
      v[k] = f(f(hi - lo) * fy + lo);
    }
    ...
    if (this.interaction) v[1] = f(v[1] * VERTICAL_ICON_WIND_SCALE);
  ...
  Particles.prototype.update = function () {
    ...
    for (var n = 0; n < 16; n++) if (p.rotation[n] !== (n % 5 === 0 ? 1 : 0)) rotate = true;
    if (!hasField) this.fieldSample.fill(0);
    for (var i = 0; i < this.capacity; i++) {
      var at = i * 12;
      if (s[at + 3] === DEAD) continue;
      if (hasField) this.sampleField(s[at], s[at + 1], s[at + 2]);
      if (rotate)
        transform4(
          f(s[at] - p.pivot[0]),
          f(s[at + 1] - p.pivot[1]),
      ...
      var ox = f(Math.sin(f(phase * f(0.37)))),
        oy = f(Math.cos(f(phase * f(0.17)))),
        oz = f(Math.cos(f(phase * f(0.31))));
      var x = s[at + 8],
        y = s[at + 9],
        z = s[at + 10],
        w = s[at + 11];
      ...
      len = f(Math.sqrt(len));
      for (k = 0; k < 4; k++) s[at + 8 + k] = f(q[k] / len);
      if (
        age >= AGE_LIMIT ||
        s[at] < p.minimum[0] ||
        s[at] > p.maximum[0] ||
  ...
  Particles.prototype.recycle = function () {
    ...
    if (!this.seedCount) return;
    for (var n = 0; n < this.retiredCount; n++) {
      var i = this.retired[n],
        j = this.seeds[i * 12 + 3] !== DEAD ? i : this.seedIndices[i % this.seedCount];
      for (var k = 0; k < 12; k++) this.state[i * 12 + k] = this.seeds[j * 12 + k];
      this.state[i * 12 + 3] = 0;
      this.recycled++;
    }
  };
  function fade(age) {
    return f(
      f(Math.min(age, f(0.02)) * 50) * f(Math.max(f(age - f(0.94)), 0) * f(-16.666664123535156) + 1)
    );
  }
  function halfTruncate(x) {
    ...
    return (
      ((u >>> 16) & 0x8000) |
      (e > 142 ? 0x7c00 : e > 112 ? ((e - 112) << 10) | ((u >>> 13) & 1023) : 0)
    );
  }
  function fromHalf(h) {
    var e = (h >>> 10) & 31;
  ...
  Particles.prototype.pack = function () {
    ...
    for (var i = 0; i < this.capacity; i++) {
      var j = i * 12;
      if (s[j + 3] === DEAD) continue;
      var o = n++ * 8;
      r[o] = s[j];
      r[o + 1] = s[j + 1];
      r[o + 2] = s[j + 2];
      r[o + 3] = fade(s[j + 3]);
      for (var k = 0; k < 4; k++) r[o + 4 + k] = fromHalf(halfTruncate(s[j + 8 + k]));
    }
    this.count = n;
    return n;
  };
  Particles.prototype.advance = function (seconds, respawn) {
    ...
    while (this.fraction >= 1 - 1e-9) {
      if (this.interaction) this.interaction.step();
      this.update();
      updates++;
      if (respawn && this.emitter) this.emitter.retire(this.retired, this.retiredCount);
      else if (respawn) this.recycle();
      this.fraction = Math.max(0, this.fraction - 1);
    }
    if (respawn && this.emitter) this.emitter.emit(updates);
    this.pack();
  };
  ...
  var api = {
    ...
    ffd: ffd,
    fade: fade,
    halfTruncate: halfTruncate,
    fromHalf: fromHalf,
    DEAD: DEAD
  };
  if (typeof module === 'object' && module.exports) module.exports = api;
  else root.LGXMBPS3Core = Object.freeze(api);
