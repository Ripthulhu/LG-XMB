...
(function (root) {
  ...
  var FILES = Object.freeze({
    ...
    category: 'snd_category_decide.wav',
    decide: 'snd_decide.wav',
  ...
  MenuSounds.prototype.getState = function () {
    return {
      ...
      phase: this.phase,
      loaded: Object.keys(this.buffers).length,
      total: Object.keys(FILES).length,
      missing: Object.keys(this.failures).map(function (key) {
        return FILES[key];
      }),
      directory: '/media/internal/lg-xmb/Sounds/',
      fallback: 'generated navigation click'
    };
  };
  ...
  MenuSounds.prototype.cancelLoads = function () {
    this.generation++;
    this.requests.forEach(function (cancel) {
      cancel();
    });
    this.requests.clear();
    this.loading = false;
    ...
    return new Promise(function (resolve) {
      ...
      function decoded(buffer) {
        ...
        if (
          ...
          buffer.length * buffer.numberOfChannels > 960000
        ) {
          finish(null);
          return;
        }
      ...
      try {
        ...
        xhr.onprogress = function (event) {
          if (event.loaded > LIMIT || (event.lengthComputable && event.total > LIMIT)) finish(null);
        };
        xhr.onerror = xhr.onabort = xhr.ontimeout = cancel;
        xhr.onload = function () {
          if (done || self.generation !== generation) {
            finish(null);
            return;
          }
  ...
  MenuSounds.prototype.prepare = function () {
    if (!this.allowed() || this.prepared || this.loading || !this.ensureContext())
      return this.ready;
    var self = this,
    ...
    this.ready = Promise.all([worker(), worker()]).then(function () {
      ...
      self.report(
        !self.enabled ? 'off' : count === keys.length ? 'ready' : count ? 'partial' : 'fallback'
      );
    });
    return this.ready;
  };
  ...
  MenuSounds.prototype.suspend = function () {
    ...
    if (this.context) {
      try {
        var result = this.context.suspend();
  ...
  MenuSounds.prototype.play = function (kind) {
    ...
    if (!this.resumePending) {
      try {
        this.resumePending = Promise.resolve(context.resume()).then(
          function () {
    ...
    this.resumePending.then(function () {
      if (context.state === 'running') trigger();
    });
    return true;
  };
  MenuSounds.prototype.trigger = function (kind) {
    var context = this.context,
      buffer = this.buffers[kind],
      source,
      gain = null,
      self = this;
    ...
    try {
      if (buffer) {
        ...
        var at = context.currentTime;
        source.type = 'sine';
        source.frequency.setValueAtTime(660, at);
        source.frequency.exponentialRampToValueAtTime(440, at + 0.065);
      ...
      while (this.voices.length >= MAX_VOICES) {
        ...
        if (old.gain) old.gain.disconnect();
      }
      ...
      source.start();
      if (!buffer) source.stop(context.currentTime + 0.07);
    } catch (ignore) {
      if (source) {
        try {
          source.disconnect();
