...
(function (root) {
  ...
  var MIN = 946684800,
    MAX = 4102444799; // Supported editor range: 2000..2099 UTC.
  ...
  function validUTC(utc) {
    return Number.isSafeInteger(utc) && utc >= MIN && utc <= MAX;
  }
  ...
  function request(method, payload) {
    ...
    var op = new Promise(function (resolve, reject) {
      ...
      cancel = function () {
        finish(problem('CANCELLED', 'Time read cancelled.'));
      };
      try {
        ...
        bridge.onservicecallback = function (raw) {
          ...
          var r;
          try {
            if (typeof raw === 'string' && raw.length > 16384) throw Error();
            r = typeof raw === 'string' ? JSON.parse(raw) : raw;
          } catch (ignore) {
            finish(problem('INVALID_RESPONSE', 'The TV returned an unreadable time response.'));
            return;
          }
          if (!r || typeof r !== 'object' || Array.isArray(r)) {
            finish(problem('INVALID_RESPONSE', 'Invalid time response.'));
            return;
          }
          if (
            r.returnValue !== true ||
            ...
            finish(
              problem('SERVICE_ERROR', clean(r.errorText) || 'The TV refused this time operation.')
            );
            return;
          }
          finish(null, r);
          ...
          finish(
            problem(
              ...
              method === 'setSystemTime'
                ? 'The TV did not confirm the change. It may have applied; read the clock before retrying.'
                : 'The TV did not return its time.'
            )
          );
        }, 5000);
        bridge.call(SERVICE + method, JSON.stringify(payload));
      } catch (ignore) {
        finish(problem('BRIDGE_ERROR', 'The TV time service is unavailable.'));
      }
    });
    op.cancel = cancel;
    return op;
  ...
  function formatter(zone) {
    ...
    var result = new Intl.DateTimeFormat('en-GB-u-nu-latn', {
      timeZone: zone,
      year: 'numeric',
      month: '2-digit',
      day: '2-digit',
  ...
  function parts(utc, zone) {
    ...
    formatter(zone)
      ...
      .forEach(function (p) {
        if (['year', 'month', 'day', 'hour', 'minute', 'second'].indexOf(p.type) >= 0)
  ...
  function localEpoch(p) {
    if (
      !p ||
      !['year', 'month', 'day', 'hour', 'minute', 'second'].every(function (k) {
        return Number.isInteger(p[k]);
      }) ||
      p.year < 2000 ||
      p.year > 2099 ||
      p.month < 1 ||
  ...
  function candidates(p, zone) {
    ...
    for (var h = -48; h <= 48; h += 12) {
      var probe = wall + h * 3600,
        offset = epoch(parts(probe, zone)) - probe;
      if (offsets.indexOf(offset) < 0) offsets.push(offset);
    }
    offsets.forEach(function (offset) {
      var utc = wall - offset;
      if (validUTC(utc) && epoch(parts(utc, zone)) === wall && result.indexOf(utc) < 0)
        result.push(utc);
    });
    ...
    if (!result.length)
      throw problem(
        'NONEXISTENT_TIME',
        'This local time does not exist in the TV time zone, or is outside the supported range.'
      );
    return result;
  }
  function normalize(r) {
    if (!Number.isSafeInteger(r.utc) || r.utc < 0 || r.utc > MAX)
      throw problem('INVALID_RESPONSE', 'The TV returned an invalid UTC time.');
    ...
    try {
      ...
      throw problem(
        ...
        'The TV time zone is not supported by this browser. Use TV Settings to adjust it.'
      );
    }
    ...
    if (!r.timezone && Number.isFinite(r.offset) && epoch(local) - r.utc !== r.offset * 60)
      throw problem(
        'TIMEZONE_MISMATCH',
        'The TV and browser time zones disagree. Use TV Settings to adjust the time zone.'
      );
  ...
  function get() {
    if (!isTV()) {
      ...
      return Promise.resolve(normalize({ utc: now, preview: true }));
    }
    var read = request('getSystemTime', {}),
      op = read.then(normalize);
    op.cancel = function () {
      read.cancel();
    };
    return op;
  ...
  function set(utc) {
    if (!validUTC(utc))
      return Promise.reject(
        problem(
          'INVALID_TIME',
          'Time must be whole Unix seconds, not milliseconds, between 2000 and 2099.'
        )
      );
    if (!isTV()) return Promise.resolve({ preview: true, utc: utc });
    if (pendingWrite)
      return Promise.reject(problem('TIME_PENDING', 'A time change is already pending.'));
    // Only explicit Apply reaches this call. Never retry a write automatically.
    var op = request('setSystemTime', { utc: utc });
    pendingWrite = op;
    var result = op.then(function () {
      return { preview: false, utc: utc };
    });
    result.then(
      function () {
